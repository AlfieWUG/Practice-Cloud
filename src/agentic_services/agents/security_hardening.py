"""
SecurityHardeningAgent - Optimizes application and infrastructure security
"""
...
class SecurityHardeningAgent(BaseAgent):
    """
    Agent responsible for security optimization post-migration.
    ...
    SYSTEM_PROMPT = """You are a security optimization expert specializing in cloud infrastructure and applications.

Your responsibilities:
1. Identify security bottlenecks
...
Provide specific, actionable security optimization recommendations with expected impact and implementation effort."""

    def __init__(self, agent_id: Optional[str] = None):
        """Initialize SecurityHardeningAgent"""
        super().__init__(agent_id)
        self.agent_type = "security_hardening"
        self.optimization_data: Optional[Dict[str, Any]] = None
    
    async def execute(self, task: Dict[str, Any]) -> Dict[str, Any]:
        """Execute security optimization analysis"""
        project_id = task.get('project_id')
        if not project_id:
            raise ValueError("project_id is required")
        
        security_assessment = task.get('security_assessment')
        if not security_assessment:
            raise ValueError("security_assessment is required")
        
        try:
            await self.emit_event(
                event_type='security_hardening.started',
                detail={},
                project_id=project_id
            )
            
            optimization_results = await self._analyze_security(project_id, security_assessment, task)
            ...
            await self.emit_event(
                event_type='security_hardening.completed',
            ...
            await self.emit_event(
                event_type='security_hardening.failed',
                detail={'error': str(e)},
                project_id=project_id
            )
            raise
    
    async def _analyze_security(self, project_id: str, security_assessment: Any, task: Dict[str, Any]) -> Dict[str, Any]:
        """Use AI to analyze security and generate optimizations"""
        prompt = f"""Analyze security metrics and provide optimization recommendations:

Security Metrics:
{json.dumps(security_assessment, indent=2)}

Provide optimization plan in JSON format with:
1. bottlenecks: Identified security bottlenecks
2. optimizations: Specific optimization recommendations
3. expected_improvements: Expected security improvements
