"""
Tests for ConfigurationAgent
"""

import pytest
from unittest.mock import patch
from agentic_services.agents.configuration import ConfigurationAgent
...
@pytest.mark.asyncio
async def test_configuration_agent_initialization():
    """Test that ConfigurationAgent initializes correctly"""
    agent = ConfigurationAgent()
    
    assert agent.agent_id is not None
    assert agent.agent_type == "configuration"
    assert agent.configuration_data is None


@pytest.mark.asyncio
async def test_configuration_agent_with_custom_id():
    """Test ConfigurationAgent initialization with custom agent_id"""
    custom_id = "test-dependency-mapper-123"
    agent = ConfigurationAgent(agent_id=custom_id)
...
@pytest.mark.asyncio
async def test_configuration_execution():
...
@pytest.mark.asyncio
async def test_configuration_with_transitive_deps():
...
@pytest.mark.asyncio
async def test_configuration_circular_detection():
...
@pytest.mark.asyncio
async def test_configuration_compatibility_assessment():
...
@pytest.mark.asyncio
async def test_configuration_vulnerability_check():
...
@pytest.mark.asyncio
async def test_configuration_critical_paths():
...
@pytest.mark.asyncio
async def test_configuration_missing_project_id():
...
@pytest.mark.asyncio
async def test_configuration_missing_resources():
    """Test error handling when resources are missing"""
    agent = ConfigurationAgent()
    ...
    with pytest.raises(ValueError, match="resources is required"):
        await agent.execute(task)


@pytest.mark.asyncio
async def test_configuration_empty_resources():
...
async def test_configuration_event_emission():
    ...
    assert 'configuration.started' in event_types
    assert 'configuration.completed' in event_types


@pytest.mark.asyncio
async def test_configuration_error_event():
...
@pytest.mark.asyncio
async def test_get_configuration_data():
    """Test retrieving dependency mapping data"""
    agent = ConfigurationAgent()
    
    # Before execution
    assert agent.get_configuration_data() is None
    ...
    configuration_data = agent.get_configuration_data()
    assert configuration_data is not None
    assert configuration_data['project_id'] == 'project-909'
    assert configuration_data == result
