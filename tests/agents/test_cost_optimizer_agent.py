"""
Tests for CostOptimizerAgent
"""

import pytest
from unittest.mock import patch
from agentic_services.agents.cost_optimizer import CostOptimizerAgent
...
@pytest.mark.asyncio
async def test_cost_optimizer_agent_initialization():
    """Test that CostOptimizerAgent initializes correctly"""
    agent = CostOptimizerAgent()
    
    assert agent.agent_id is not None
    assert agent.agent_type == "cost_optimizer"
    assert agent.optimization_data is None


@pytest.mark.asyncio
async def test_cost_optimizer_agent_with_custom_id():
    """Test CostOptimizerAgent initialization with custom agent_id"""
    custom_id = "test-dependency-mapper-123"
    agent = CostOptimizerAgent(agent_id=custom_id)
...
@pytest.mark.asyncio
async def test_cost_optimization_execution():
...
@pytest.mark.asyncio
async def test_cost_optimization_with_transitive_deps():
...
@pytest.mark.asyncio
async def test_cost_optimization_circular_detection():
...
@pytest.mark.asyncio
async def test_cost_optimization_compatibility_assessment():
...
@pytest.mark.asyncio
async def test_cost_optimization_vulnerability_check():
...
@pytest.mark.asyncio
async def test_cost_optimization_critical_paths():
...
@pytest.mark.asyncio
async def test_cost_optimization_missing_project_id():
    """Test error handling when project_id is missing"""
    agent = CostOptimizerAgent()
    
    task = {
        'cost_metrics': [{'name': 'app'}]
    }
...
@pytest.mark.asyncio
async def test_cost_optimization_missing_cost_metrics():
    """Test error handling when cost_metrics are missing"""
    agent = CostOptimizerAgent()
    ...
    with pytest.raises(ValueError, match="cost_metrics is required"):
        await agent.execute(task)


@pytest.mark.asyncio
async def test_cost_optimization_empty_cost_metrics():
    """Test handling of empty cost_metrics list"""
    agent = CostOptimizerAgent()
...
@pytest.mark.asyncio
async def test_cost_optimization_state_persistence():
    """Test that mapping state is persisted to DynamoDB"""
    agent = CostOptimizerAgent()
    
    task = {
        'project_id': 'project-606',
        'cost_metrics': [{'name': 'test-app'}]
    }
...
@pytest.mark.asyncio
async def test_cost_optimization_event_emission():
    """Test that appropriate events are emitted during mapping"""
    agent = CostOptimizerAgent()
    
    task = {
        'project_id': 'project-707',
        'cost_metrics': [{'name': 'app-1'}, {'name': 'app-2'}]
    }
    ...
    assert 'cost_optimization.started' in event_types
    assert 'cost_optimization.completed' in event_types


@pytest.mark.asyncio
async def test_cost_optimization_error_event():
