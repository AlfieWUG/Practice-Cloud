"""
Tests for SecurityHardeningAgent
"""

import pytest
from unittest.mock import patch
from agentic_services.agents.security_hardening import SecurityHardeningAgent
...
@pytest.mark.asyncio
async def test_security_hardening_agent_initialization():
    """Test that SecurityHardeningAgent initializes correctly"""
    agent = SecurityHardeningAgent()
    
    assert agent.agent_id is not None
    assert agent.agent_type == "security_hardening"
    assert agent.optimization_data is None


@pytest.mark.asyncio
async def test_security_hardening_agent_with_custom_id():
    """Test SecurityHardeningAgent initialization with custom agent_id"""
    custom_id = "test-dependency-mapper-123"
    agent = SecurityHardeningAgent(agent_id=custom_id)
...
@pytest.mark.asyncio
async def test_security_hardening_execution():
...
@pytest.mark.asyncio
async def test_security_hardening_with_transitive_deps():
...
@pytest.mark.asyncio
async def test_security_hardening_circular_detection():
...
@pytest.mark.asyncio
async def test_security_hardening_compatibility_assessment():
...
@pytest.mark.asyncio
async def test_security_hardening_vulnerability_check():
...
@pytest.mark.asyncio
async def test_security_hardening_critical_paths():
...
@pytest.mark.asyncio
async def test_security_hardening_missing_project_id():
    """Test error handling when project_id is missing"""
    agent = SecurityHardeningAgent()
    
    task = {
        'security_assessment': [{'name': 'app'}]
    }
...
@pytest.mark.asyncio
async def test_security_hardening_missing_security_assessment():
    """Test error handling when security_assessment are missing"""
    agent = SecurityHardeningAgent()
    ...
    with pytest.raises(ValueError, match="security_assessment is required"):
        await agent.execute(task)


@pytest.mark.asyncio
async def test_security_hardening_empty_security_assessment():
    """Test handling of empty security_assessment list"""
    agent = SecurityHardeningAgent()
...
@pytest.mark.asyncio
async def test_security_hardening_state_persistence():
    """Test that mapping state is persisted to DynamoDB"""
    agent = SecurityHardeningAgent()
    
    task = {
        'project_id': 'project-606',
        'security_assessment': [{'name': 'test-app'}]
    }
...
@pytest.mark.asyncio
async def test_security_hardening_event_emission():
    """Test that appropriate events are emitted during mapping"""
    agent = SecurityHardeningAgent()
    
    task = {
        'project_id': 'project-707',
        'security_assessment': [{'name': 'app-1'}, {'name': 'app-2'}]
    }
    ...
    assert 'security_hardening.started' in event_types
    assert 'security_hardening.completed' in event_types


@pytest.mark.asyncio
async def test_security_hardening_error_event():
