...
render_professional_header(
    "Analytics Dashboard",
    "Real-time insights into agent performance, migrations, and system health"
)
...
if recent_activity:
    for activity in recent_activity[:10]:
        ...
        # Status by action type (professional text labels)
        action_labels = {
            'project_created': 'New',
        ...
        label = action_labels.get(activity.get('action_type', ''), 'Activity')
        ...
        with st.container():
            ...
            with col1:
                if user_name:
                    st.markdown(f"**{label}:** {description} (by {user_name})")
                else:
